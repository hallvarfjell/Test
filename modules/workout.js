const Workout = {
  ...
  render(el, st){
    ...
    const piCol = UI.h('div',{class:'kpi-col'}, UI.h('div',{class:'kpi-label'},'PI / Slope'), UI.h('div',{class:'kpi-val', id:'pi'}, '0'), UI.h('div',{class:'kpi-val', id:'slope'}, '0.00'));
    ...
    let idx=0, left=0, started=false, paused=false, timerHandle=null; let tTot=0, tDrag=0, dist=0; const hrSamples=[], spdSamples=[]; const hrBuf=[];
    ...
    function updatePI(){ const slope=calcSlope(); const pi = (typeof PI!=='undefined')? PI.compute({ hr: (hrBuf.length?hrBuf[hrBuf.length-1].bpm:0), hrmax: st.settings.hrmax, speedKmh: st.tm.speed||0, inclinePct: st.tm.incline||0, lt1: st.settings.lt1, lt2: st.settings.lt2 }): 0; document.getElementById('pi').textContent = String(pi); document.getElementById('slope').textContent = slope.toFixed(2); }

    function tick(){ if(paused) return; tTot++; tDrag++; document.getElementById('tTot').textContent=UI.fmtTime(tTot); document.getElementById('tDrag').textContent=UI.fmtTime(tDrag); document.getElementById('timer').textContent=UI.fmtTime(left); document.getElementById('clock2').textContent=new Date().toLocaleTimeString(); const cur = seq[idx]; const spdNow = (cur.kind==='Pause')? 0 : (st.tm.speed||0); document.getElementById('spd').textContent = `${spdNow.toFixed(1)} km/t`; Graph.addSPD(spdNow); dist += spdNow/3600; document.getElementById('dist').textContent = dist.toFixed(2); if(cur.kind==='Arbeid'){ if(st.hr.bpm) hrSamples.push(st.hr.bpm); spdSamples.push(spdNow); } updatePI(); left--; if(left<=0){ if(cur.kind==='Arbeid'){ const aHR = Math.round(hrSamples.reduce((a,b)=>a+b,0)/(hrSamples.length||1)); const aSpd = (spdSamples.reduce((a,b)=>a+b,0)/(spdSamples.length||1)).toFixed(1); document.getElementById('avgHR').textContent = String(aHR); document.getElementById('avgSpd').textContent = String(aSpd); hrSamples.length=0; spdSamples.length=0; } idx++; setPhase(); } }
