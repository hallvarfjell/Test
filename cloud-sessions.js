// cloud-sessions.js – unified build: robust queue + deleteSession() with queue suppression
...
export async function pullSessions(){
  ...
  (data||[]).forEach(r=>{ if(!byId.has(r.client_id)){
    byId.set(r.client_id,{ id:r.client_id, name:r.name||'Økt', startedAt:r.started_at, endedAt:r.started_at, reps:r.reps||0, lt1:r.lt1??null, lt2:r.lt2??null, massKg:r.mass_kg??null, points:[] });
  }});
...
async function pushOne(entry){
  ...
  const payload={ user_id, client_id:session.id, name:session.name||'Økt', started_at:session.startedAt, ended_at:session.endedAt,
    duration_sec:Math.max(1,Math.round((new Date(session.endedAt)-new Date(session.startedAt))/1000)), reps:Number(session.reps||0),
    lt1:Number(session.lt1??null), lt2:Number(session.lt2??null), mass_kg:Number(session.massKg??null),
    distance_m:Number(pts.at(-1)?.dist_m??0), elev_gain_m:Number(session.metrics?.elevGainM??0), tss:Number(session.metrics?.tss??0),
...
window.__INTZ_SUPPRESS_QUEUE__ = false;
...
(function(){
  ...
  const watcher=(k,v)=>{ if(window.__INTZ_SUPPRESS_QUEUE__) return; if(isSessionsKey(k)){ enqueueFromValue(v); processQueue(); } };
  if(!watchers.includes(watcher)) watchers.push(watcher);
})();

function enqueueFromValue(v){ let arr=[]; try{ arr=JSON.parse(v||'[]'); }catch(_){ arr=[]; }
  const q=loadQ(); const known=new Set(q.map(x=>x.id));
  arr.forEach(s=>{ if(!s||!s.id||!s.startedAt) return; if(!known.has(s.id)) q.push(s); });
  saveQ(q);
}
function scanAllKeys(){ const q0=loadQ(); let qset=new Set(q0.map(x=>x.id));
  for(let i=0;i<localStorage.length;i++){
    const k=localStorage.key(i);
    if(isSessionsKey(k)){
      try{ const arr=JSON.parse(localStorage.getItem(k)||'[]');
        (arr||[]).forEach(s=>{ if(s&&s.id&&!qset.has(s.id)){ q0.push(s); qset.add(s.id); } });
      }catch(_){ }
    }
  }
  saveQ(q0);
...
export async function deleteSession(client_id){
  ...
  try{
    window.__INTZ_SUPPRESS_QUEUE__ = true;
  ...
  finally{ window.__INTZ_SUPPRESS_QUEUE__ = false; }
}
