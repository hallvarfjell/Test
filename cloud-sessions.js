// cloud-sessions.js – HOTFIX-2 (diag + fallback scan): auto-push + kø + periodisk skanning av nsKey-sessions
...
export async function pullSessions(){
  ...
  const { data, error } = await supabase.from('workouts').select('client_id,name,started_at,reps,lt1,lt2,mass_kg').order('started_at',{ascending:false});
...
async function pushOne(entry){
  ...
  console.log('[sessions] storage upload', {path, error:up.error});
  ...
  console.log('[sessions] db upsert', upsert);
