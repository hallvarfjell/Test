...
(function(){ 
 ...
 const msg = (e && e.stack) ? e.stack : (e?.message 
\
 String(e)); 
...
\
 ...
 function displaySpeedKmh(){ if(STATE.workout && (STATE.workout.phase==='rest' 
\
 STATE.workout.phase==='seriesrest')) return 0; return STATE.speedKmh; } 
...
0,0,10); 
 ...
 function applyRPEChange(delta){ setRPE((Number(el('rpe-now')?.value)
\
0)+delta); } 
 ...
 name:cw.name
\
'Økt', phase:'warmup', startedAt:null, endedAt:null, 
 warmupSec:Number(cw.warmupSec)
\
0, cooldownSec:Number(cw.cooldownSec)
\
0, 
 series:(cw.series
\
[]).map(s=>({reps:s.reps,workSec:s.workSec,restSec:s.restSec,seriesRestSec:s.seriesRestSec,note:s.note
\
''})), 
 sIdx:-1, rep:0, tLeft:Number(cw.warmupSec)
\
0 
 } 
 } 
 ...
 const s=cfg.series
\
[]; let total=Number(cfg.warmupSec
\
0)+Number(cfg.cooldownSec
\
0); 
 for(const x of s){ total += Number(x.reps
\
0) * (Number(x.workSec
\
0) + Number(x.restSec
\
0)); total += Number(x.seriesRestSec
\
0); } 
 return total; 
 } 
 ...
 opt.value='c:'+idx; opt.textContent=cw.name
\
('Mal '+(idx+1)); sel.appendChild(opt); 
 }); 
 ...
 if(ar.type==='custom'){ 
 const arr=loadCustomWorkouts(); const i=Number(ar.index
\
0); const cw=arr[i]; 
 if(cw){ 
 ...
 const icon=el('sp-icon'), label=el('sp-label'); if(!icon
\
!label) return; 
 ...
 w.phase='work'; w.sIdx=0; w.rep=1; w.tLeft=w.series[0].workSec
\
0; 
 if(w.tLeft===0){ w.phase='rest'; w.tLeft=w.series[0].restSec
\
0; } 
 ...
 if(w.rep < s.reps){ w.phase='rest'; w.tLeft=s.restSec
\
0; return; } 
...
0; 
 ...
 w.sIdx++; w.phase='work'; w.rep=1; w.tLeft=w.series[w.sIdx].workSec
\
0; 
 if(w.tLeft===0){ w.phase='rest'; w.tLeft=w.series[w.sIdx].restSec
\
0; } 
 return; 
 } 
 ...
 const s=w.series[w.sIdx]; 
 if(w.rep < s.reps){ 
 w.rep++; w.phase='work'; w.tLeft=s.workSec
\
0; 
 if(w.tLeft===0){ if(w.rep<=s.reps){ w.phase='rest'; w.tLeft=s.restSec
\
0; } } 
 return; 
 } 
...
0; } 
 ...
 if(w.phase==='work'){ 
 const s=w.series[w.sIdx]; 
 if(w.rep>1){ w.phase='rest'; w.rep--; w.tLeft=s.restSec
\
0; } 
 else{ 
 if(w.sIdx>0){ 
 w.sIdx--; const ps=w.series[w.sIdx]; 
 w.phase='work'; w.rep=ps.reps; w.tLeft=ps.workSec
\
0; 
 if(w.tLeft===0){ w.phase='rest'; w.tLeft=ps.restSec
\
0; } 
 ...
 const s=w.series[w.sIdx]; w.phase='work'; w.tLeft=s.workSec
\
0; 
 if(w.tLeft===0){ w.phase='rest'; w.tLeft=s.restSec
\
0; } 
 } 
 ...
 if(prev){ 
 w.sIdx--; w.phase='work'; w.rep=prev.reps; w.tLeft=prev.workSec
\
0; 
 if(w.tLeft===0){ w.phase='rest'; w.tLeft=prev.restSec
\
0; } 
 }else{ w.phase='warmup'; w.tLeft=w.warmupSec; } 
 } 
 ...
 if(last){ 
 w.phase='work'; w.sIdx=w.series.length-1; w.rep=last.reps; w.tLeft=last.workSec
\
0; 
 if(w.tLeft===0){ w.phase='rest'; w.tLeft=last.restSec
\
0; } 
 }else{ w.phase='warmup'; w.tLeft=w.warmupSec; } 
 } 
...
0)-1); 
 ...
 const g=9.81, v=(speedKmh
\
0)/3.6, grade=(gradePct
\
0)/100; 
 const mech = massKg * (g * v * grade + Crun * v); 
 return Math.max(0, Math.round(mech * (K
\
1))); 
 } 
 ...
 ts:t, iso:new Date(t).toISOString(), hr:STATE.hr
\
0, speed_ms, 
 grade:STATE.gradePct
\
0, dist_m:STATE.logger.dist, rpe:STATE.rpe, 
...
\
 ...
 const session={ 
 id:'s'+Date.now(), name:w.name
\
'Økt', 
 reps:(w.series
\
[]).reduce((a,s)=>a+(Number(s.reps)
\
0),0), 
 ...
 if(w.phase==='seriesrest') return fmtMMSS(w.series[w.sIdx].seriesRestSec
\
0); 
 if(w.phase==='rest') return fmtMMSS(w.series[w.sIdx].restSec
\
0); 
 if(w.phase==='work') return fmtMMSS(w.series[w.sIdx].workSec
\
0); 
 return ''; 
 } 
 ...
 obj.phase='work'; obj.sIdx=0; obj.rep=1; obj.tLeft=obj.series[0].workSec
\
0; 
 if(obj.tLeft===0){ obj.phase='rest'; obj.tLeft=obj.series[0].restSec
\
0; } 
 ...
 if(obj.rep < s.reps){ obj.phase='rest'; obj.tLeft=s.restSec
\
0; return; } 
 if(obj.sIdx < obj.series.length-1){ 
 const sr=s.seriesRestSec
\
0; if(sr>0){ obj.phase='seriesrest'; obj.tLeft=sr; return; } 
 obj.sIdx++; obj.phase='work'; obj.rep=1; obj.tLeft=obj.series[obj.sIdx].workSec
\
0; 
 if(obj.tLeft===0){ obj.phase='rest'; obj.tLeft=obj.series[obj.sIdx].restSec
\
0; } 
 return; 
 } 
 ...
 const s=obj.series[obj.sIdx]; 
 if(obj.rep < s.reps){ 
 obj.rep++; obj.phase='work'; obj.tLeft=s.workSec
\
0; 
 if(obj.tLeft===0){ obj.phase='rest'; obj.tLeft=s.restSec
\
0; } 
 return; 
 } 
...
!cs){ return; } 
 ...
 : (w.phase==='seriesrest')? (w.series[w.sIdx].seriesRestSec
\
0) 
 : (w.phase==='rest')? (w.series[w.sIdx].restSec
\
0) 
 : (w.phase==='work')? (w.series[w.sIdx].workSec
\
0) 
 : 1; 
...
plotH<=0) return; 
 ...
 const HR = (STATE.series.hr
\
[]).filter(p=>p.t>=xmin); 
 const WT = (STATE.series.watt
\
[]).filter(p=>p.t>=xmin); 
 const SP = (STATE.series.speed
\
[]).filter(p=>p.t>=xmin); 
 const RP = (STATE.series.rpe
\
[]).filter(p=>p.t>=xmin); 
 ...
 if(!lockS){ const pad=(sMax-sMin)*0.05
\
0.5; sMin-=pad; sMax+=pad; if(sMax<=sMin) sMax=sMin+0.5; } 
 ...
 function drawLine(arr, color, ymap, alpha=1){ if(!arr
\
arr.length<2) return; ctx.strokeStyle=color; ctx.globalAlpha=alpha; ctx.lineWidth=2*dpr; ctx.beginPath(); let moved=false; for(const p of arr){ if(p.t<xmin) continue; const x=xTime(p.t), y=ymap(p.y); if(!moved){ ctx.moveTo(x,y); moved=true;} else ctx.lineTo(x,y);} ctx.stroke(); ctx.globalAlpha=1; } 
 ...
 // Ghost overlay 
 const ghostEnabled = document.getElementById('ghost-enable')?.checked 
\
 false; 
 ...
 const a20=avgSince(STATE.series.hr
\
[], now-20000), a120=avgSince(STATE.series.hr
\
[], now-120000); 
 ...
 try{ document.getElementById('err-card')?.classList.remove('hidden'); const L=document.getElementById('err-log'); if(L) L.textContent += (e?.stack
\
 e?.message
\
 String(e))+"\n"; }catch(_){} 
...
[]); 
 ...
 const pts=s.points
\
[]; if(!pts.length) return {dur:0, hr:[], w:[]}; 
 ...
 const p=pts[idx]; hr[sec]=p.hr
\
0; w[sec]=Math.round(p.watt
\
0); 
 } 
 return {dur, hr, w}; 
...
[])) 
 ...
 btn.addEventListener('click', (ev)=> setGrade(Number(ev.currentTarget.dataset.grade
\
0)) ); 
 // no-HR modal 
 ...
 canvas=el('chart'); ctx=canvas?.getContext('2d'); dpr=window.devicePixelRatio
\
1; 
 ...
 STATE.ghost.ids=new Set(Array.from(checks
\
...
function updateLivePanelINTZ(){
  try{
    ...
    const W=window.STATE; const w=W.workout;
    const fmt=(sec)=>{sec=Math.max(0,Math.floor(sec||0)); const m=Math.floor(sec/60),ss=String(sec%60).padStart(2,'0'); return `${m}:${ss}`;};
    const totalPlanned=(()=>{ try{ if(w&&typeof w.totalSec==='number'&&isFinite(w.totalSec)&&w.totalSec>0) return w.totalSec; let tot=(w?.warmupSec||0)+(w?.cooldownSec||0); for(const x of (w?.series||[])){ tot += (Number(x.reps||0))*((Number(x.workSec||0))+(Number(x.restSec||0))); tot += Number(x.seriesRestSec||0);} return tot; }catch(_){return 0;}})();
    if(w && w.startedAt){ const es=Math.max(0,Math.floor((Date.now()-new Date(w.startedAt).getTime())/1000)); const rs=Math.max(0,(totalPlanned||es)-es); if($('live-elapsed')) $('live-elapsed').textContent=fmt(es); if($('live-remaining')) $('live-remaining').textContent=fmt(rs);} 
    ...
    if(!W.metrics||!W.metrics.lastDrag){ try{ const pts=W.logger?.points||[]; if(pts.length){ let end=pts.length-1; if(pts[end].phase==='work'){} else { while(end>=0&&pts[end].phase!=='work') end--; if(end>=0){ let start=end; while(start>0&&pts[start-1].phase==='work') start--; const seg=pts.slice(start,end+1); if(seg.length){ const avgSpd=seg.reduce((a,p)=>a+(p.speed_ms||0),0)/seg.length; const endTs=seg[seg.length-1].ts; const hr20=seg.filter(p=>p.ts>=endTs-20000).map(p=>p.hr||0); const avgHr20=hr20.length? Math.round(hr20.reduce((a,b)=>a+b,0)/hr20.length):0; W.metrics=W.metrics||{}; W.metrics.lastDrag={speedKmh:(avgSpd||0)*3.6, hr20:avgHr20}; } } } }catch(_){}}
    if($('last-drag-line') && W.metrics && W.metrics.lastDrag){ const ld=W.metrics.lastDrag; $('last-drag-line').textContent='Siste drag: snittfart '+(ld.speedKmh||0).toFixed(1)+' km/t · snittpuls (20s) '+(ld.hr20||0)+' bpm'; }
  }catch(_){ }
}
...
;(function(){
  try{
    if (typeof window.updateLivePanelINTZ==='function'){
      if (typeof window.writeSample==='function' && !window.writeSample._intzWrapped){
        const _orig=window.writeSample; const _wrap=function(t){ try{ return _orig(t);} finally{ try{ updateLivePanelINTZ(); }catch(e){} } }; _wrap._intzWrapped=true; window.writeSample=_wrap;
      }
      if (typeof window.startTicker==='function' && !window.startTicker._intzWrapped){
        const __orig=window.startTicker; const __wrap=function(){ const r=__orig.apply(this, arguments); try{ updateLivePanelINTZ(); }catch(e){} return r; }; __wrap._intzWrapped=true; window.startTicker=__wrap;
      }
    }
  }catch(e){}
})();
