// Økt v1.4.5 – hurtigvalg (konfigurerbart), graf (grid), Slope
function stdWorkouts(){
  return [
    { key:'6x5_WU12_CD6', name:'6×5 (WU12, CD6, pause 1)', blocks:[{kind:'Oppvarming',dur:12*60},{kind:'Intervall',reps:6,work:5*60,rest:60},{kind:'Nedjogg',dur:6*60}] },
    { key:'6x6_WU7_CD7',  name:'6×6 (WU7, CD7, pause 1)',  blocks:[{kind:'Oppvarming',dur:7*60},{kind:'Intervall',reps:6,work:6*60,rest:60},{kind:'Nedjogg',dur:7*60}] },
    { key:'8x6_WU7_CD7',  name:'8×6 (WU7, CD7, pause 1)',  blocks:[{kind:'Oppvarming',dur:7*60},{kind:'Intervall',reps:8,work:6*60,rest:60},{kind:'Nedjogg',dur:7*60}] }
  ];
}
...
const WorkoutEngine=(function(){ const S={ running:false, paused:false, timer:null, plan:null, seq:[], idx:0, left:0, tTot:0, dist:0, rpeCur:6,
  hrSeries:[], spdSeries:[], incSeries:[], _perDrag:[], lastDrag:{hr:'-',spd:'-',watt:'-',rpe:'-',inc:'-'}, P2:null, tssLoad:0 };
  function reset(){ S.running=false; S.paused=false; if(S.timer){clearInterval(S.timer);S.timer=null;} S.seq=[];S.idx=0;S.left=0;S.tTot=0;S.dist=0;S.hrSeries=[];S.spdSeries=[];S.incSeries=[];S._perDrag=[];S.lastDrag={hr:'-',spd:'-',watt:'-',rpe:'-',inc:'-'}; S.tssLoad=0; S.P2=(AppState.settings?.P_L2)||null; }
  ...
  function hrAvgLast(sec){ const now=Date.now()/1000; const arr=S.hrSeries.filter(p=>p.t>=now-sec).map(p=>p.bpm).filter(Boolean); if(!arr.length) return NaN; return arr.reduce((a,b)=>a+b,0)/arr.length; }
  ...
  function updateUI(){ const spd=AppState.tm?.speed||0, inc=AppState.tm?.incline||0, hr=AppState.hr?.bpm||0; const pow=(function(){ const mass=(AppState.settings?.mass)||75; const v=spd/3.6, i=(inc||0)/100; const Wkg=(4.185*v+(9.81*v*i)/((AppState.settings?.met_eff)||0.25))*(1-((AppState.settings?.shoe_gain_pct||0)/100))*((AppState.settings?.tm_cal)||1); return {Wkg, W:Wkg*mass}; })();
    ...
    const h=document.getElementById('wk_hr'); if(h) h.textContent=String(hr||0);
    const slopeEl=document.getElementById('wk_slope'); if(slopeEl){ const a20=hrAvgLast(20), a120=hrAvgLast(120); if(isFinite(a20)&&isFinite(a120)&&a120>0){ const s=( (a20-a120)/a120 )*100; slopeEl.textContent=(s>=0?'+':'')+s.toFixed(1)+'%'; slopeEl.style.color = s>1? '#d93c3c' : (s<-1? '#2dbb55' : '#4c5672'); } else slopeEl.textContent='–%'; }

    const s=document.getElementById('wk_spd'); if(s) s.textContent=`${spd.toFixed(1)} km/t`; const iEl=document.getElementById('wk_inc'); if(iEl) iEl.textContent=`${Math.round(inc)}%`; const w=document.getElementById('wk_pow'); if(w) w.textContent=`${Math.round(pow.W)} W`; const tss=document.getElementById('wk_tss'); if(tss){ const v=(S.tssLoad/3600)*100; tss.textContent=String(Math.round(v)); }
    ...
    const aHR=document.getElementById('wk_avgHR'); if(aHR) aHR.textContent=String(S.lastDrag.hr); const aS=document.getElementById('wk_avgSpd'); if(aS) aS.textContent=String(S.lastDrag.spd); const aW=document.getElementById('wk_avgPow'); if(aW) aW.textContent=String(S.lastDrag.watt); const rpe=document.getElementById('wk_rpe'); if(rpe) rpe.textContent=(S.rpeCur||6).toFixed(1);
  }
  ...
  function drawGraph(){ const canvas=document.getElementById('wk_canvas'); if(!canvas) return; const ctx=canvas.getContext('2d'); const W=canvas.width, H=canvas.height; ctx.clearRect(0,0,W,H);
    // grid
    ctx.strokeStyle='#eef3fb'; ctx.lineWidth=1; [0.25,0.5,0.75].forEach(f=>{ ctx.beginPath(); ctx.moveTo(0,Math.round(H*f)); ctx.lineTo(W,Math.round(H*f)); ctx.stroke(); });
    ...
    function plot(series,color,accessor,yMin,yMax){ if(!series.length) return; ctx.strokeStyle=color; ctx.lineWidth=1.6; ctx.beginPath(); series.forEach((p,idx)=>{ const x=mapX(p.t); const v=accessor(p); const y=H-5- ( (v-yMin)/Math.max(1,(yMax-yMin)) * (H-10) ); if(idx===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); }); ctx.stroke(); }
    plot(hr,'#e74c3c',p=>p.bpm||0,60,200);
    plot(spd,'#0d6efd',p=>p.kmh||0,0,20);
    plot(inc,'#f59e0b',p=>p.pct||0,0,15);
    if(S.paused){ ctx.fillStyle='rgba(0,0,0,.05)'; ctx.fillRect(0,0,W,H); ctx.fillStyle='#4c5672'; ctx.fillText('PAUSE', W-60, 14); }
  }
...
const Workout={ onHR:null, onTM:null, render(el,st){ el.innerHTML='';
  // Standardøkter og valg
  const std=stdWorkouts();
  const last=Storage.loadP(AppState.currentProfile,'last_plan_key', std[0].key);
  let plan = std.find(p=>p.key===last) || std[0];
  ...
  const left=UI.h('div',{});
  left.append(UI.h('div',{class:'small'},'Puls'));
  left.append(UI.h('div',{id:'wk_hr',style:'font-size:3rem;font-weight:700'},'0'));
  left.append(UI.h('div',{class:'small'},'Slope'));
  left.append(UI.h('div',{id:'wk_slope',style:'font-size:1.2rem;color:#4c5672'},'–%'));
  ...
  const spdHeader=UI.h('div',{class:'controls'});
  const quickCfg=UI.h('button',{class:'btn',title:'Rediger hurtigvalg'},UI.icon('ph-gear')); 
  spdHeader.append(UI.h('div',{class:'small'},'Hurtigvalg'), quickCfg);
  const spdQuick=UI.h('div',{class:'controls',id:'wk_quick'});
  function loadQuick(){ const def=[6,8,10,12,14,16]; const arr=Storage.loadP(AppState.currentProfile,'quick_speeds',def); spdQuick.innerHTML=''; (arr||def).forEach(v=>{ spdQuick.append(UI.h('button',{class:'btn',onclick:()=>{AppState.tm.speed=v; AppState.tm.manualUntil=Date.now()+4000;}}, `${v} km/t`)); }); }
  loadQuick();
  quickCfg.onclick=()=>{ const def=Storage.loadP(AppState.currentProfile,'quick_speeds',[6,8,10,12,14,16]); const inp=prompt('Hurtigvalg (km/t), kommadelt:', def.join(',')); if(!inp) return; const arr=inp.split(',').map(s=>parseFloat(s)).filter(x=>!isNaN(x)&&x>0).slice(0,10); if(arr.length){ Storage.saveP(AppState.currentProfile,'quick_speeds',arr); loadQuick(); } };
  mid.append(spdCtl, spdHeader, spdQuick);
  ...
  const planSel=UI.h('select',{id:'wk_plan'});
  std.forEach(p=>{ const o=document.createElement('option'); o.value=p.key; o.textContent=p.name; if(p.key===plan.key) o.selected=true; planSel.appendChild(o); });
  ...
  box.append(UI.h('div',{},UI.h('div',{class:'small'},'Øktplan'), planSel), UI.h('div',{class:'controls'},toggle,prev,next,save,disc), UI.h('div',{class:'controls'}, UI.h('div',{},UI.h('div',{class:'small'},'Totaltid'), UI.h('div',{id:'wk_tot'},'00:00')), UI.h('div',{},UI.h('div',{class:'small'},'Distanse (km)'), UI.h('div',{id:'wk_dist'},'0.00'))));
  ...
  planSel.addEventListener('change', ()=>{ const key=planSel.value; const p=std.find(x=>x.key===key)||std[0]; Storage.saveP(AppState.currentProfile,'last_plan_key', key); WorkoutEngine.init(p); });
