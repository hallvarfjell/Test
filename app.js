// INTZ v1.4.3 – robust router (fix: don't rely on window.* for const)
const AppState = {
...
const Routes = {
  '#/dashboard': (el,st)=> (typeof Dashboard!=='undefined' && Dashboard.render) ? Dashboard.render(el,st) : (el.textContent='Dashboard mangler.'),
  '#/editor'   : (el,st)=> (typeof Editor   !=='undefined' && Editor.render)    ? Editor.render(el,st)    : (el.textContent='Editor mangler.'),
  '#/workout'  : (el,st)=> (typeof Workout  !=='undefined' && Workout.render)   ? Workout.render(el,st)   : (document.getElementById('app').innerHTML='<div class="card">Økt‑modulen kunne ikke lastes.<br>Kontroller at <code>modules/workout.js</code> lastes før <code>app.js</code>.</div>'),
  '#/result'   : (el,st)=> (typeof Result   !=='undefined' && Result.render)    ? Result.render(el,st)    : (el.textContent='Resultat mangler.'),
  '#/pi'       : (el,st)=> (typeof PIMod    !=='undefined' && PIMod.render)     ? PIMod.render(el,st)     : (el.textContent='PI-modul mangler.'),
  '#/log'      : (el,st)=> (typeof LogMod   !=='undefined' && LogMod.render)    ? LogMod.render(el,st)    : (el.textContent='Logg mangler.')
};
...
window.addEventListener('load',()=>{ populateProfileSel(); reloadStateForProfile(); router(); ensureWakeLock(); const clock=document.getElementById('clock'); if(clock) setInterval(()=>{ clock.textContent=new Date().toLocaleTimeString(); },1000); const bHR=document.getElementById('btn-hr'); if(bHR) bHR.addEventListener('click', async()=>{ try{ await BT.connectHR(bpm=>{ AppState.hr.bpm=bpm; AppState.hr.connected=true; UI.setConnected('btn-hr',true); if(typeof Workout!=='undefined' && Workout.onHR) Workout.onHR(bpm); }); }catch(e){ alert('HR tilkobling feilet: '+e.message); } }); const bTM=document.getElementById('btn-ftms'); if(bTM) bTM.addEventListener('click', async()=>{ try{ await BT.connectFTMS((spd,inc)=>{ if(typeof Workout!=='undefined' && Workout.onTM) Workout.onTM(spd,inc); AppState.tm.connected=true; UI.setConnected('btn-ftms',true); }); }catch(e){ alert('FTMS tilkobling feilet: '+e.message); } }); const bEx=document.getElementById('btn-export'); if(bEx) bEx.addEventListener('click', ()=>{ const data={ ver:'1.4.3', exportedAt:new Date().toISOString(), items:{} }; for(let i=0;i<localStorage.length;i++){ const k=localStorage.key(i); if(k&&k.startsWith('INTZ_')){ try{ data.items[k]=JSON.parse(localStorage.getItem(k)); }catch(_){ data.items[k]=localStorage.getItem(k); } } } const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='intz_backup_'+Date.now()+'.json'; a.click(); URL.revokeObjectURL(a.href); }); const bIm=document.getElementById('btn-import'); if(bIm) bIm.addEventListener('click', ()=>{ const inp=document.createElement('input'); inp.type='file'; inp.accept='.json,application/json'; inp.onchange=async()=>{ const f=inp.files[0]; if(!f) return; try{ const data=JSON.parse(await f.text()); if(data&&data.items){ Object.entries(data.items).forEach(([k,v])=> localStorage.setItem(k, JSON.stringify(v))); location.reload(); } else alert('Ugyldig backup.'); }catch(e){ alert('Import feilet: '+e.message);} }; inp.click(); }); });